...
import { recordScan } from '@firestoreService/scanning/scanStatsService';
import { auth } from '@config/firebase';
import {
  isGuestUser,
  incrementGuestScanCount,
  getGuestRemainingScans
} from '@utils/guest/guestScanUtils';
import { decrementScanCount, getUsageLimits } from '@firestoreService/subscription/subscriptionService';
...
export default function ScanScreen({ navigation }) {
  ...
  const handlePinchGesture = (event) => {
    ...
    if (scale > 1) setZoom(prev => Math.min(prev + 0.02, 1));
    else if (scale < 1) setZoom(prev => Math.max(prev - 0.02, 0));
  };
  ...
  const handleCapture = async () => {
    if (!isCameraReady || !cameraRef.current || isProcessing) return;
  ...
  const analyzeImage = async (base64Image, photoUri) => {
    try {
      ...
      setIsProcessing(false);
      Alert.alert("Error", "Could not analyze the image. Please check your internet connection.");
    }
  };
  ...
  const isPlantOrAnimal = (visionData) => {
    ...
    for (const label of labels) {
      ...
      if (plantAnimalKeywords.some(kw => desc.includes(kw))) relevantScore += score;
      if (irrelevantKeywords.some(kw => desc.includes(kw))) irrelevantScore += score;
    }
  ...
  const extractCandidates = (visionData) => {
    ...
    webLabels.forEach(item => {
      ...
      if (text && text.length > 2 && !genericTerms.has(text.toLowerCase())) candidates.add(text);
    });
  ...
  const detectCategoryFromLabels = (visionData) => {
    ...
    for (const label of labels) {
      ...
      if (plantKeywords.some(kw => desc.includes(kw))) plantScore += score;
      if (animalKeywords.some(kw => desc.includes(kw))) animalScore += score;
    }
  ...
  const identifyWithPlantNet = async (photoUri) => {
    try {
      ...
      formData.append('images', { uri: photoUri, type: 'image/jpeg', name: 'plant.jpg' });
      ...
      console.error('PlantNet failed:', error.message);
      return null;
    }
  };
  ...
  const fetchTaxonDetails = async (taxonId) => {
    try {
      const response = await axios.get(`https://api.inaturalist.org/v1/taxa/${taxonId}`, { timeout: 6000 });
      return response?.data?.results?.[0] || null;
    } catch (error) {
      return null;
    }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.permissionContainer}>
          ...
          <TouchableOpacity onPress={handleGalleryPick} style={styles.galleryPermissionButton}>
      ...
      <PinchGestureHandler onGestureEvent={handlePinchGesture}>
        <View style={styles.container}>
          ...
          <View style={styles.rightControls}>
            ...
            <TouchableOpacity style={[styles.controlButton, { marginTop: 10 }]} onPress={toggleAutofocus}>
              <Ionicons name={autofocus === 'on' ? 'scan' : 'scan-outline'} size={24} color="#fff" />
            </TouchableOpacity>
          </View>
          ...
          {/* ✅ BUTTON CONTAINER WITH GALLERY BUTTON */}
