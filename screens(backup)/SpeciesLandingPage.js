...
import { decrementDownloadCount, getUsageLimits } from '@firestoreService/subscription/subscriptionService';
import PremiumGate from '@components/PremiumGate';
import { createDownloadNotification } from '@firestoreService/notifications/notificationService';
  ...
  try {
    ...
    if (favoritesResult.success) {
      ...
      if (existing) {
        ...
        console.log('Species is already in favorites');
      } else {
        setIsFavorite(false);
        setFavoriteId(null);
      }
...
const toggleFavorite = async () => {
  ...
  try {
    ...
    if (isFavorite && favoriteId) {
      ...
      const { getFavorites } = require('@firestoreService');
      const favoritesResult = await getFavorites(uid);
